...
def get_backscatter_mol_attn_v1(alphamol, betamol, alt, idxref):
    ...
    for i in range(len(alt)-2, idxref, -1):
...
### Determiner z0
zbottom = opts.zbottom #17700
ztop = opts.ztop #19000

wavelengths = [355, 532]

maindir = Path('/homedata/nmpnguyen/ORACLES-ER2/')
listpaths = sorted(maindir.glob('*R8.h5'))
for file_er2 in listpaths[4:]:
    print(file_er2)
    er2alt = xr.open_dataset(file_er2, group='DataProducts')['Altitude'][:].values.flatten() 
    er2time = xr.open_dataset(file_er2, group='Nav_Data')['gps_time'][:].values.flatten()
    new_signal = np.array([processed(wave, ztop, zbottom, file_er2)[0] for wave in wavelengths])
    new_simul = np.array([processed(wave, ztop, zbottom, file_er2)[1] for wave in wavelengths])
    ...
    ds.to_netcdf(Path(maindir,"/RF/Calibrated/", file_er2.name), 'a')