...
for i in range(10, len(files)):
    ...
    signal2 = pd.DataFrame((df.variables['signal'][:,:z2.shape[0],5]))
    for t in time2:
        t = pd.to_datetime(t)
        ...
        plt.savefig(get_path_out(main_path = "/homedata/nmpnguyen/OPAR/Fig", time_index = t, lidar_name = "LI1200", channel = "profil2channels", arranging = "day"))
...
for i in range(0, len(files)):
    ...
    print(i, file)
    df = nc4.Dataset(file, 'r')    
    signal_sum = df.variables['signal'][:,:,5]
    ...
    if get_signal_significatif(signal) is None:
        ...
        plt.savefig(get_path_out(main_path = "/homedata/nmpnguyen/OPAR/Fig", time_index = time_index, lidar_name = "LI1200", channel = "2channels", arranging = "month"))
        plt.clf()
        plt.close()
